...
分析：
    ...
    6.把切割好的序号和文本的内容存储到HashMap集合中（key的序号是有序的，会自动排序）；
...
public class Demo05Test {
    public static void main(String[] args) throws IOException {
        //创建一个HashMap集合对象，key存储每行文本的序号（1，2，3...），value存储每行的文本
        HashMap<String, String> map = new HashMap<>();
        //创建字符缓冲输入流对象，构造方法中绑定字符输入流
        BufferedReader br = new BufferedReader(new FileReader("G:\\b\\c.txt"));
        //创建字符缓冲输出流对象，构造方法中绑定字符输出流
        BufferedWriter bw = new BufferedWriter(new FileWriter("G:\\b\\d.txt"));
        ...
        for (String key : map.keySet()) {
            ...
            //把每一个键值对拼接为一个文本行
            line = key + "." + value;
            //把拼接好的文本行使用字符缓冲输出流中的方法write，写入到文件中
            bw.write(line);
            bw.newLine(); //写换行
        }
